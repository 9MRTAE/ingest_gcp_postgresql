...
# ── Identity ──────────────────────────────────────────────────────────────────
DB_NAME   = 'facility_management_api_prd'
GCS_APPLICATION = 'facilityv2' # maps to gcp-storage-parquet/{GCS_APPLICATION}/postgresql/{table}/... existing folder in GCS bucket
...
TABLE_REGISTRY: dict[str, dict] = {
    ...
    'facility_item_operate_settings': {
        'p_createdate': 'created_date',
        'p_updatedate': 'updated_date',
    },
    'facility_items': {
...
def ingest_gcp_postgresql_facility_management_api_prd(
    TABLE_NAME: str = '',
    JOB_BACKDATE_DEPEN: str = JOB_BACKDATE,
    JOB_STARTDATE_DEPEN: str = JOB_STARTDATE,
    JOB_ENDDATE_DEPEN: str = JOB_ENDDATE,
) -> dict[str, str]:
    """Ingest tables from facility_management_api_prd → GCS Data Lake.
