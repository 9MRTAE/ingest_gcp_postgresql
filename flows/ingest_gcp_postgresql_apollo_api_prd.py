"""
Flow  : ingest_gcp_postgresql_apollo_api_prd
Repo  : ingest_gcp_postgresql_ecoapp
DB    : GCP · PostgreSQL · your-db-hostname · apollo_api_prd

GCS output path:
    {bucket}/gcp-storage-parquet/apollo_api_prd/postgresql/{table}/
    calendar_year=YYYY/month_no=M/day_of_month=D/

Run all tables (normal / scheduled):
    python flows/ingest_gcp_postgresql_apollo_api_prd.py

Repair single table:
    python flows/ingest_gcp_postgresql_apollo_api_prd.py --table alembic_version
    python flows/ingest_gcp_postgresql_apollo_api_prd.py --table alembic_version --backdate -3
    python flows/ingest_gcp_postgresql_apollo_api_prd.py --table alembic_version --startdate 2024-01-01 --enddate 2024-01-31
"""
...
# ── Identity ──────────────────────────────────────────────────────────────────
DB_NAME         = 'apollo_api_prd'
GCS_APPLICATION = 'servicemaintenance'   # TODO: fill in GCS application folder name
FLOW_NAME       = f'ingest_{PLATFORM}_{DB_TYPE}_{DB_NAME}'

# ── Table Registry ────────────────────────────────────────────────────────────
# เพิ่ม/ลด table ที่นี่ — ไม่ต้องแตะ flow function
# key   : table name ใน source DB
# value : extract keyword args (p_createdate, p_updatedate)
TABLE_REGISTRY: dict[str, dict] = {
    ...
    'report_service_maintenance_work_request_daily': {
        'p_createdate': 'created_date',
        'p_updatedate': 'updated_date',
    },
    'service_maintenance_expense': {
        'p_createdate': 'created_date',
        'p_updatedate': 'updated_date',
    },
    'service_maintenance_expense_log': {
        'p_createdate': 'created_date',
        'p_updatedate': 'updated_date',
    },
    'service_maintenance_next_number': {
        'p_createdate': 'created_date',
        'p_updatedate': 'updated_date',
    },
    'service_maintenance_review': {
        'p_createdate': 'created_date',
        'p_updatedate': 'updated_date',
    },
    ...
    'service_maintenance_type': {
        'p_createdate': 'created_date',
        'p_updatedate': 'updated_date',
    },
    'service_maintenance_work_request': {
        'p_createdate': 'created_date',
        'p_updatedate': 'updated_date',
    },
    'service_maintenance_work_request_log': {
        'p_createdate': 'created_date',
        'p_updatedate': 'updated_date',
    },
    'technician': {
