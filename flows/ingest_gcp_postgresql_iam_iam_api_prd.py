"""
Flow  : ingest_gcp_postgresql_iam_api_prd
Repo  : ingest_gcp_postgresql_ecoapp
DB    : GCP · PostgreSQL · your-db-hostname · iam_api_prd

GCS output path:
    {bucket}/gcp-storage-parquet/iam_api_prd/postgresql/{table}/
    calendar_year=YYYY/month_no=M/day_of_month=D/

Run all tables (normal / scheduled):
    python flows/ingest_gcp_postgresql_iam_api_prd.py

Repair single table:
    python flows/ingest_gcp_postgresql_iam_api_prd.py --table companies
    python flows/ingest_gcp_postgresql_iam_api_prd.py --table companies --backdate -3
    python flows/ingest_gcp_postgresql_iam_api_prd.py --table companies --startdate 2024-01-01 --enddate 2024-01-31
"""
...
# ── Identity ──────────────────────────────────────────────────────────────────
DB_NAME         = 'iam_iam_api_prd'
GCS_APPLICATION = 'pmsmanagement'   # TODO: fill in GCS application folder name
...
TABLE_REGISTRY: dict[str, dict] = {
    ...
    'invitation_companies': {
        'p_createdate': 'responded_at',
        'p_updatedate': 'responded_at',
    },
    'invitation_projects': {
        'p_createdate': 'responded_at',
        'p_updatedate': 'responded_at',
