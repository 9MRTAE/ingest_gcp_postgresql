"""
Flow  : ingest_gcp_postgresql_notification_notification_api_prd
Repo  : ingest_gcp_postgresql_ecoapp
DB    : GCP · PostgreSQL · your-db-hostname · notification_notification_api_prd

GCS output path:
    {bucket}/gcp-storage-parquet/notification_notification_api_prd/postgresql/{table}/
    calendar_year=YYYY/month_no=M/day_of_month=D/

Run all tables (normal / scheduled):
    python flows/ingest_gcp_postgresql_notification_notification_api_prd.py

Repair single table:
    python flows/ingest_gcp_postgresql_notification_notification_api_prd.py --table activity_log
    python flows/ingest_gcp_postgresql_notification_notification_api_prd.py --table activity_log --backdate -3
    python flows/ingest_gcp_postgresql_notification_notification_api_prd.py --table activity_log --startdate 2024-01-01 --enddate 2024-01-31
"""
...
# ── Identity ──────────────────────────────────────────────────────────────────
DB_NAME         = 'notification_notification_api_prd'
GCS_APPLICATION = 'notification'   # TODO: fill in GCS application folder name
...
def ingest_gcp_postgresql_notification_notification_api_prd(
    TABLE_NAME:          str = '',
    JOB_BACKDATE_DEPEN:  str = JOB_BACKDATE,
    JOB_STARTDATE_DEPEN: str = JOB_STARTDATE,
    JOB_ENDDATE_DEPEN:   str = JOB_ENDDATE,
) -> dict[str, str]:
    """Ingest tables from notification_notification_api_prd → GCS Data Lake.
